...
export class ProjectDetector {
  ...
  static async detectProjectType(files: { name: string; content: string }[]): Promise<DetectedProject> {
    ...
    const htmlFiles = Array.from(fileMap.keys()).filter(name => name.toLowerCase().endsWith('.html'));
    if (htmlFiles.length > 0) {
      detectedFiles.push(...htmlFiles);
    }
    ...
    for (const [type, indicators] of Object.entries(this.PROJECT_INDICATORS)) {
      ...
      if (type === 'static' && htmlFiles.length > 0 && !packageJson) {
        confidence = 0.8;
        typeDetectedFiles.push(...htmlFiles);
      }
